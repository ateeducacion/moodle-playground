...
if [ -d "$SOURCE_DIR/.git" ]; then
  echo "Updating cached source for $BRANCH (ref: $GIT_REF)" >&2
  (cd "$SOURCE_DIR" && git fetch --depth 1 origin "$GIT_REF" >&2 && git reset --hard FETCH_HEAD >&2)
else
  echo "Cloning $GITHUB_REPO ref $GIT_REF into $BRANCH" >&2
  rm -rf "$SOURCE_DIR"
  git clone --depth 1 --branch "$GIT_REF" "$GITHUB_REPO" "$SOURCE_DIR" >&2
fi
