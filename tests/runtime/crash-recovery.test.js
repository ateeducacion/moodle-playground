...
describe("crash recovery request handler", () => {
   ...
   *
   * Only reactive restarts are performed (no preventive rotation).
   * An anti-loop guard prevents restarts if fewer than
   * MIN_REQUESTS_BEFORE_RESTART requests have been processed.
   */
  function createMockWorker() {
    // MAX_REACTIVE is lower than the real MAX_REACTIVE_RESTARTS (20) so
    // tests can exhaust the budget quickly.  MIN_REQUESTS_BEFORE_RESTART
    ...
    function resetRuntime(reason) {
      if (reactiveRestartCount >= MAX_REACTIVE) {
        messages.push({
          kind: "error",
      ...
      if (requestCount < MIN_REQUESTS_BEFORE_RESTART) {
        messages.push({
          kind: "error",
          detail: `crash after only ${requestCount} requests, not restarting: ${reason}`,
        });
        return false;
      }
    ...
    return {
      ...
      /** Simulate processing N requests to pass the anti-loop guard. */
      simulateRequests(n) {
        requestCount += n;
      },
    };
  ...
  it("auto-retries a failed GET request on fatal WASM error", async () => {
    const worker = createMockWorker();
    worker.simulateRequests(15); // Pass anti-loop guard
  ...
  it("handles crash on retry gracefully (no infinite loop)", async () => {
    ...
    assert.ok(result.body.includes("crashed again on retry"));
    assert.strictEqual(result.retried, true);
    // Only one restart: the retry crash's resetRuntime is blocked by the
    // anti-loop guard (requestCount was reset to 0 after the first restart).
    assert.strictEqual(worker.reactiveRestartCount, 1);
  });
  ...
  it("does NOT restart if fewer than MIN_REQUESTS_BEFORE_RESTART processed (anti-loop guard)", async () => {
    const worker = createMockWorker();
    // Only 3 requests processed — well below the minimum of 10
    worker.simulateRequests(3);
    worker.setCrashOnRequest(true);
    const result = await worker.handleRequest({
      method: "GET",
      url: "http://localhost/page",
    });
    // resetRuntime returns false due to anti-loop guard, no retry
    assert.strictEqual(result.status, 503);
    assert.strictEqual(worker.reactiveRestartCount, 0); // Not incremented
    // Message should mention the guard
    assert.ok(worker.messages.some((m) => m.detail.includes("not restarting")));
  });

  it("restarts normally once MIN_REQUESTS_BEFORE_RESTART is reached", async () => {
    const worker = createMockWorker();
    worker.simulateRequests(10); // Exactly at the threshold
    worker.setCrashOnRequest(true);
    const result = await worker.handleRequest({
      method: "GET",
      url: "http://localhost/page",
    });
    // Should recover: guard passes, retry succeeds
    assert.strictEqual(result.status, 200);
    assert.strictEqual(result.retried, true);
    assert.strictEqual(worker.reactiveRestartCount, 1);
  });
...
describe("createSnapshotManager", () => {
  function createMockPostShell() {
    const messages = [];
    return {
      postShell: (msg) => messages.push(msg),
      messages,
    };
  }
  ...
  it("returns an object with the expected API", () => {
    ...
    assert.strictEqual(typeof sm.hydrate, "function");
    assert.strictEqual(typeof sm.restore, "function");
    assert.strictEqual(typeof sm.clear, "function");
    assert.strictEqual(typeof sm.hasPendingRestore, "boolean");
  });

  it("starts with no pending restore", () => {
    const { postShell } = createMockPostShell();
    const sm = createSnapshotManager({ postShell });
  ...
  it("hydrate() handles read errors gracefully", async () => {
    const { postShell, messages } = createMockPostShell();
    const sm = createSnapshotManager({ postShell });
    const mockPhp = {
      _php: {
        readFileAsBuffer: () => {
    ...
    await sm.hydrate(mockPhp, "/persist/test.sq3.php");
    assert.strictEqual(sm.hasPendingRestore, false);
    assert.ok(messages.some((m) => m.detail?.includes("failed to read DB")));
  });

  it("hydrate() saves snapshot when read succeeds", async () => {
    const { postShell, messages } = createMockPostShell();
    const sm = createSnapshotManager({ postShell });
    const fakeData = new Uint8Array([1, 2, 3, 4]);
    const mockPhp = {
      _php: {
        readFileAsBuffer: () => fakeData.buffer,
      },
    };
    await sm.hydrate(mockPhp, "/persist/test.sq3.php");
    assert.strictEqual(sm.hasPendingRestore, true);
    assert.ok(messages.some((m) => m.detail?.includes("saved DB")));
  });
});
